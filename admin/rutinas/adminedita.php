...
$stmt = $conexion->prepare("SELECT * FROM rutinas WHERE id_rutina = ? AND tipo = 'oficial'");
...
$deportes = ["Todos", "Atletismo", "Fútbol", "Baloncesto", "Pádel", "Ciclismo", "Natación", "Tenis"];
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($errores)){
        // UPDATE con sentencia preparada (PDO)
        $stmt = $conexion->prepare(
            "UPDATE rutinas SET titulo=?, descripcion=?, deporte=?, dificultad=?, duracion_minutos=?
             WHERE id_rutina=? AND tipo='oficial'"
        );
        $stmt->execute([
            $datos['titulo'],
            $datos['descripcion'],
        ...
        // Borrar ejercicios anteriores y reinsertar los nuevos (ya saneados)
...
<div class="container py-5" style="max-width: 800px;">
    <div class="card shadow border-0 p-4">
        <h2 class="fw-bold mb-4" style="color: var(--rojo-mezquita);">
            <i class="fa-solid fa-pen me-2"></i>Editar Rutina Oficial
        </h2>
                ...
                <div class="col-md-4">
                    ...
                    <select name="deporte" class="form-select" required>
                        <?php foreach($deportes as $d): ?>
                            <option value="<?php echo $d; ?>" <?php echo $rutina['deporte'] === $d ? 'selected' : ''; ?>>
                                <?php echo $d; ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
